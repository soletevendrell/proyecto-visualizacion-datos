procesar_csv <- function(ruta_csv) {
  # --- 0. Leer CSV ----------------------------------------------------------
  df <- read.csv(
    file = ruta_csv,
    sep = ";",
  ...
  # --- 1. Crear Cod anio ---------------------------------------------------
  if (!is.null(df[[1]])) {
    df[["Cod anio"]] <- substr(as.character(df[[1]]), 1, 4)
  } else {
  ...
  # --- 4. Reordenar: dejar Cod anio como segunda columna --------------------
  idx_cod_anio <- which(names(df) == "Cod anio")
  ...
  key_cols <- c("Cod anio", "Cod municipio")
  ...
  tmp_ext[["Cod anio"]] <- as.character(tmp_ext[["Cod anio"]])
  tmp_ext[["Cod municipio"]] <- as.character(tmp_ext[["Cod municipio"]])
  
  # Orden y extracción de mínimos por grupo
  ord_min <- with(tmp_ext, order(tmp_ext[["Cod anio"]], tmp_ext[["Cod municipio"]], col9, na.last = TRUE))
  tmp_min <- tmp_ext[ord_min, , drop = FALSE]
  min_df <- tmp_min[!duplicated(tmp_min[c("Cod anio", "Cod municipio")]), , drop = FALSE]
  
  # Orden y extracción de máximos por grupo
  ord_max <- with(tmp_ext, order(tmp_ext[["Cod anio"]], tmp_ext[["Cod municipio"]], -col9, na.last = TRUE))
  tmp_max <- tmp_ext[ord_max, , drop = FALSE]
  max_df <- tmp_max[!duplicated(tmp_max[c("Cod anio", "Cod municipio")]), , drop = FALSE]
  ...
  # --- 7. AGRUPACIÓN por Cod anio y Cod municipio --------------------------
  df <- df[order(as.character(df[["Cod anio"]]), as.character(df[["Cod municipio"]])), , drop = FALSE]
  ...
  if (nrow(min_df) > 0) {
    df_agg <- merge(df_agg, min_df[, c(key_cols, nombre_col_min, "Cod min_mes"), drop = FALSE],
  ...
  if (nrow(max_df) > 0) {
    df_agg <- merge(df_agg, max_df[, c(key_cols, nombre_col_max, "Cod max_mes"), drop = FALSE],
                    by = key_cols, all.x = TRUE)
  } else {
  ...
  col_order <- c(
    ...
    "Cod anio",
    "Cod municipio",
    setdiff(cols_current, c(primera_col_name, "Cod anio", "Cod municipio"))
  )
  col_order <- unique(col_order[col_order %in% cols_current])
  df_agg <- df_agg[, col_order, drop = FALSE]
  
  # --- NUEVO: eliminar la última columna del resultado si existe -----------
  if (ncol(df_agg) >= 1) {
    df_agg <- df_agg[, -ncol(df_agg), drop = FALSE]
  } else {
    warning("El data.frame resultado no tiene columnas; no se puede eliminar la última columna.")
  }
  
  # --- NUEVO: reordenar filas por Cod municipio y luego por Cod anio -------
  if (all(c("Cod municipio", "Cod anio") %in% names(df_agg))) {
    # forzamos a character para evitar problemas con factores/números
    df_agg[["Cod municipio"]] <- as.character(df_agg[["Cod municipio"]])
    df_agg[["Cod anio"]] <- as.character(df_agg[["Cod anio"]])
    df_agg <- df_agg[order(df_agg[["Cod municipio"]], df_agg[["Cod anio"]]), , drop = FALSE]
  } else {
    warning("No se pudo reordenar por 'Cod municipio' y 'Cod anio' porque no existen ambas columnas en el resultado.")
  }
